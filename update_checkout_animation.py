        ...
        <div className="feed-info relative z-20 mt-auto px-4 pb-6 space-y-2 max-w-[95%] w-full mx-auto">
          ...
          <div className="mt-2 animate-in slide-in-from-bottom duration-500 delay-300 relative group/commerce flex flex-col items-stretch gap-0">
            <div className="w-full z-30 mb-0 pb-0 relative">
              ...
              {isEditMode && (
                <div className="bg-white/10 backdrop-blur-xl border border-b-0 border-[#FFC107] rounded-t-xl p-2 pb-0 mb-0 shadow-2xl animate-in slide-in-from-bottom fade-in duration-300 relative mx-0 z-10 w-full mb-0 !rounded-b-none">
...
# 1. Remove old FeedItem (It might already be replaced from last script, so we find the new signature)
start_marker = "interface FeedItemProps {"
end_marker = "export const CheckoutFeed: React.FC<CheckoutFeedProps> ="

start_idx = content.find(start_marker)
end_idx = content.find(end_marker)

if start_idx != -1 and end_idx != -1:
    # Replace the entire FeedItem block
    content = content[:start_idx] + new_feed_item_code + "\n\n" + content[end_idx:]

# 2. Update CheckoutFeed to pass setShouldAnimateAbsorb
usage_old = """shouldAnimateAbsorb={shouldAnimateAbsorb}
              handleMpesa={handleMpesa}"""
usage_new = """shouldAnimateAbsorb={shouldAnimateAbsorb}
              setShouldAnimateAbsorb={setShouldAnimateAbsorb}
              handleMpesa={handleMpesa}"""

content = content.replace(usage_old, usage_new)

# 3. Remove the Effect related to auto animation
effect_str = """  // Trigger animation when form BECOMES filled (from Jan 24 logic)
  useEffect(() => {
    if (isFormFilled) {
      setShouldAnimateAbsorb(true);
...
print("CheckoutFeed.tsx successfully updated with manual animation triggers.")
