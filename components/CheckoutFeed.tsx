...
export const CheckoutFeed: React.FC<CheckoutFeedProps> = ({ user, onBuyIntent, onProfileClick }) => {
  ...
  // CHECKOUT BOTTOM SHEET (Replaces separate page)
  const CheckoutSheet = () => {
    ...
    return (
        <div className="absolute inset-0 z-[60] flex flex-col justify-end">
            {/* Backdrop -> Clicking closes it */}
            <div 
                className="absolute inset-0 bg-black/40 backdrop-blur-[2px] animate-in fade-in duration-300"
                onClick={() => setIsCheckoutOpen(false)}
            />
            
            {/* Sheet Content - Ultra Thin (20% screen target) */}
            <div className="relative bg-white/95 backdrop-blur-xl w-full rounded-t-3xl px-4 pt-3 pb-3 shadow-2xl animate-in slide-in-from-bottom duration-300 border-t border-white/50">
                {/* Drag Handle - Minimal */}
                <div className="w-8 h-1 bg-slate-300/50 rounded-full mx-auto mb-3" />
                
                {/* Close Button - Tucked tight */}
                <button 
                    onClick={() => setIsCheckoutOpen(false)}
                    className="absolute top-3 right-3 p-1.5 bg-slate-100/80 rounded-full text-slate-400 hover:bg-slate-200 transition-colors"
                >
                    <X size={14} />
                </button>

                {/* IN-FLOW INPUTS (Super Condensed) */}
                <div className="space-y-2 mb-3">
                    {/* Phone Input */}
                    <div className="bg-slate-50/80 p-2.5 rounded-xl border border-slate-100 flex items-center gap-3">
                        <div className="bg-white p-1.5 rounded-lg text-emerald-600 shadow-sm shrink-0 border border-slate-100">
                            <Smartphone size={16} />
                        </div>
                        <div className="flex-1 min-w-0">
                            <label className="block text-[8px] font-extrabold text-slate-400 uppercase tracking-widest">M-Pesa</label>
                            <input 
                                type="tel"
                                value={phone}
                                onChange={(e) => setPhone(e.target.value)}
                                className="w-full font-bold text-sm text-slate-900 bg-transparent outline-none -mt-0.5 placeholder:text-slate-300"
                            />
                        </div>
                    </div>
                    ...
                    <div className="bg-slate-50/80 p-2.5 rounded-xl border border-slate-100 flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                             <label className="block text-[8px] font-extrabold text-slate-400 uppercase tracking-widest">Location</label>
                            <input 
                                type="text"
                                value={location}
                ...
                <div className="space-y-2">
                    <button 
                        ...
                        className="w-full h-12 bg-[#4CAF50] hover:bg-[#43A047] text-white rounded-xl font-bold text-sm uppercase tracking-wide shadow-xl shadow-green-600/20 flex items-center justify-center gap-2 active:scale-[0.98] transition-all"
                    >
                         {isProcessing ? (
                    ...
                    <div className="flex justify-center items-center gap-1 text-slate-400">
                        <ShieldCheck size={10} />
                        <span className="text-[8px] font-bold uppercase tracking-wider">SokoTrust Escrow</span>
                    </div>
                </div>
            </div>
        </div>
